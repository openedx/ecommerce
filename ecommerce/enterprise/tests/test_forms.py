...
from ecommerce.enterprise.constants import BENEFIT_MAP, BENEFIT_PROXY_CLASS_MAP
...
class EnterpriseOfferFormTests(EnterpriseServiceMockMixin, TestCase):
    ...
    def assert_enterprise_offer_conditions(self, offer, enterprise_customer, enterprise_customer_catalog_uuid,
                                           expected_benefit_value, expected_benefit_type, expected_name):
        ...
        self.assertEqual(offer.condition.enterprise_customer, enterprise_customer)
    ...
    def test_init(self):
        ...
        self.assertEqual(
            ...
            enterprise_offer.condition.enterprise_customer['uuid'].hex
        )
        self.assertEqual(
            form['enterprise_customer_catalog_uuid'].value(),
            enterprise_offer.condition.enterprise_customer_catalog_uuid.hex
        )
        self.assertEqual(form['benefit_type'].value(), BENEFIT_PROXY_CLASS_MAP[enterprise_offer.benefit.proxy_class])
    ...
    def test_clean_with_conflicting_enterprise_customer_and_catalog_uuids(self):
        ...
        data = self.generate_data(
            enterprise_customer={
                'name': 'BigEnterprise',
                'uuid': offer.condition.enterprise_customer['uuid'],
            },
            enterprise_customer_uuid=offer.condition.enterprise_customer['uuid'],
    ...
    def test_save_create(self):
        ...
        self.assert_enterprise_offer_conditions(
            ...
            data['benefit_value'],
            data['benefit_type'],
            "Discount for BigEnterprise's Catalog with UUID [{}]".format(data['enterprise_customer_catalog_uuid']),
        )
    ...
    def test_save_create_special_char_title(self):
        ...
        self.assert_enterprise_offer_conditions(
            ...
            data['benefit_value'],
            data['benefit_type'],
            "Discount for Spánish Enterprise's Catalog with UUID [{}]".format(data['enterprise_customer_catalog_uuid'])
        )
    ...
    def test_save_offer_name(self):
        """ If a request object is sent, the offer name should include the enterprise name and catalog UUID. """
        data = self.generate_data()
        self.mock_specific_enterprise_customer_api(data['enterprise_customer_uuid'])
        form = EnterpriseOfferForm(request=self.request, data=data)
        form.is_valid()
    ...
    def test_create_when_conditional_offer_with_uuid_exists(self):
        ...
        factories.EnterpriseOfferFactory(
            condition__enterprise_customer=data['enterprise_customer'],
