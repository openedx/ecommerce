...
def update_order():
    try:
        ...
        print("The order {} does not exist.".format(ORDER_NUMBER))
        return
    ...
    LinePrice.object.filter(order=order).update(quantity=1)

    PaymentEventQuantity.objects.filter(lines__in=order.objects.all()).update(quantity=1)

    # Make sure it does not update the lines whom baskets were deleted.
    if order.basket:
        BasketLine.objects.filter(basket=order.basket)
