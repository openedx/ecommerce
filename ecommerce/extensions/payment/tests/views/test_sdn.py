...
class SDNCheckViewTests(TestCase):
    ...
    def test_sdn_check_missing_args(self):
        response = self.client.post(self.sdn_check_path, headers=self.token)
    ...
    def test_sdn_check_search_fails_uses_fallback(self, mock_search, mock_fallback):
        ...
        response = self.client.post(self.sdn_check_path, data=self.post_params, headers=self.token)
