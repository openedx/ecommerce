...
class EnterpriseCouponOverviewListSerializer(serializers.ModelSerializer):
    ...
    """
    def _get_num_unassigned(self, coupon):
        """
        Return number of available assignments.
        """
        all_slots_available = 0
        vouchers = coupon.attr.coupon_vouchers.vouchers.all().prefetch_related('offers')
        enterprise_offer = vouchers.first().enterprise_offer

        assignments = OfferAssignment.objects.select_related(
            'offer__condition',
            'offer__benefit'
        ).filter(code__in=vouchers.values_list('code', flat=True)).exclude(
            status__in=[OFFER_REDEEMED, OFFER_ASSIGNMENT_REVOKED]
        ).values('code').annotate(na=Count('code')).order_by('code')

        vouchers_num_assignments = {item['code']: item['na'] for item in assignments}

        for voucher in vouchers:
            num_assignments = vouchers_num_assignments.get(voucher.code, 0)
            voucher_slots_available = voucher.calculate_available_slots(enterprise_offer, num_assignments)
    ...
    def to_representation(self, coupon):  # pylint: disable=arguments-differ
        ...
        data = {
            ...
            'num_unassigned': self._get_num_unassigned(coupon),
