...
class EnterpriseCouponViewSetRbacTests(
    ...
    def setUp(self):
        ...
        if role:
            ...
            EcommerceFeatureRoleAssignment.objects.get_or_create(
                role=role,
                user=self.user,
                enterprise_id='f5c9149f-8dce-4410-bb0f-85c0f2dda864'
            )
            EcommerceFeatureRoleAssignment.objects.get_or_create(
                role=role,
                user=self.user,
                enterprise_id='f5c9149f-8dce-4410-bb0f-85c0f2dda860'
            )
        self.set_jwt_cookie(
    ...
    def tearDown(self):
        super(EnterpriseCouponViewSetRbacTests, self).tearDown()
        httpretty.disable()
        httpretty.reset()
    ...
    def test_permission_denied_coupon_overview(self):
        """
        Test that we get access denied via role assignment
        """
        Switch.objects.update_or_create(name=ENTERPRISE_OFFERS_FOR_COUPONS_SWITCH, defaults={'active': True})
        response = self.get_response('POST', ENTERPRISE_COUPONS_LINK, self.data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
