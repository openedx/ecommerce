...
class UtilsTests(CourseCatalogTestMixin, PartnerMixin, TestCase):
    def test_generate_sku_for_course_seat(self):
        ...
        product = course.create_or_update_seat(certificate_type, False, 0, partner)

        _hash = u'{} {} {} {} {}'.format(certificate_type, course_id, 'False', '', partner.id)
        _hash = md5(_hash.lower()).hexdigest()[-7:]
        # verify that generated sku has partner 'short_code' as prefix
        expected = _hash.upper()
        actual = generate_sku(product, partner)
