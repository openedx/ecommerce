...
This is pretty much the same as the Oscar file, except we lazily load the strategy. This ensures that we do not attempt
to load the strategy before migrations have been run, which would be bad since our strategy depends on the site table
existing.
"""
...
class ProductIndex(indexes.SearchIndex, indexes.Indexable):
    ...
    def prepare_price(self, obj):
        result = None
        if obj.is_parent:
            result = self.strategy.fetch_for_parent(obj)
        elif obj.has_stockrecords:
            result = self.strategy.fetch_for_product(obj)
