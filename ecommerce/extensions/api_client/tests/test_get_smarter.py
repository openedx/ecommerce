...
class TestGetSmarterEnterpriseApiClient(TestCase):
    ...
    @ddt.data(
        (datetime.now(pytz.utc).timestamp() + 60, 'bcde'),
        (datetime.now(pytz.utc).timestamp() - 60, 'abcd'),
    )
    @ddt.unpack
    @mock.patch('ecommerce.extensions.api_client.get_smarter.TieredCache')
    @responses.activate
    def test_cached_access_token(self, expires_at, expected_token, mock_tiered_cache):
        """
