...
from ecommerce.extensions.basket.constants import EMAIL_OPT_IN_ATTRIBUTE, ENABLE_STRIPE_INTENTS_PAYMENT_PROCESSOR
...
class PaymentApiResponseTestMixin(BasketLogicTestMixin):
    ...
    def assert_expected_response(
            self,
            basket,
            enable_stripe_intents_payment_processor=False,
            url=None,
            response=None,
            status_code=200,
            product_type=u'Seat',
        ...
        expected_response = {
            'basket_id': basket.id,
            'currency': currency,
            'enable_stripe_intents_payment_processor': enable_stripe_intents_payment_processor,
            'offers': offers,
            'coupons': coupons,
...
class PaymentApiViewTests(PaymentApiResponseTestMixin, BasketMixin, DiscoveryMockMixin,
    ...
    @ddt.data(True, False)
    def test_enable_stripe_intents_payment_processor_flag(self, enable_stripe_intents_payment_processor):
        with override_flag(ENABLE_STRIPE_INTENTS_PAYMENT_PROCESSOR, active=enable_stripe_intents_payment_processor):
            seat = self.create_seat(self.course)
            basket = self.create_basket_and_add_product(seat)
            response = self.client.get(self.path)
            self.assert_expected_response(
