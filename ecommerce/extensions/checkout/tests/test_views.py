...
class FreeCheckoutViewTests(TestCase):
    ...
    @ddt.data(('{}?order_number={}', True), ('{}?orderNum={}', False))
    @ddt.unpack
    @httpretty.activate
    def test_successful_redirect(self, expected_url, isSwitchToggled):
        """ Verify redirect to the receipt page. """
        toggle_switch('otto_receipt_page', isSwitchToggled)
        self.prepare_basket(0)
        self.assertEqual(Order.objects.count(), 0)
        if isSwitchToggled:
            receipt_page = self.site.siteconfiguration.build_ecommerce_url(settings.RECEIPT_PAGE_PATH)
        else:
        ...
        expected_url = expected_url.format(receipt_page, Order.objects.first().number)
