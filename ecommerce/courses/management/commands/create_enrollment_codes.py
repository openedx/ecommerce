...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            dest='batch_offset',
            default=0,
            help='Number of courses to skip for enrollment code creation.',
            type=int,
        )
        parser.add_argument(
            '--batch-limit',
        ...
        parser.add_argument(
            '--batch-sleep-time',
            action='store',
            dest='batch_sleep_time',
            default=3,
            help='Sleep time (in minutes) between the processing batches for courses.',
            type=int,
        )
    ...
    def handle(self, *args, **options):
        ...
        batch_sleep_time = options['batch_sleep_time']
        ...
        if course_ids_file:
            ...
            total_courses, failed_courses = self._generate_enrollment_codes_from_db(
                batch_limit,
                batch_offset,
                batch_sleep_time,
            )
    ...
    def _generate_enrollment_codes_from_db(self, batch_limit, batch_offset, batch_sleep_time):
        """
        Generate enrollment codes for the course.

        Arguments:
            batch_limit (int): How many courses to fetch from db to process in each batch.
            batch_offset (int): Offset for fetching course batch from db.
            batch_sleep_time (int): Time in seconds to sleep before processing next batch of courses.
        ...
        courses = Course.objects.all()[batch_offset:batch_offset + batch_limit]
        while len(courses) > 0:
            total_courses += len(courses)
            batch_offset += courses.count()
            ...
            time.sleep(batch_sleep_time * 60)
            courses = Course.objects.all()[batch_offset:batch_offset + batch_limit]
