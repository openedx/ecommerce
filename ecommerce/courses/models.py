...
class Course(models.Model):
    ...
    def create_or_update_seat(self, certificate_type, id_verification_required, price, partner,
        ...
        # Create (or update) the corresponding bulk purchase coupon product for this seat, if requested.
        if create_bulk_product:
            coupon_title = unicode('Bulk Purchase: {seat_title}'.format(seat_title=seat.title))
            coupon_categories = ProductCategory.objects.filter(product=seat)
            self.create_or_update_coupon_product(
                title=coupon_title,
                price=price,
                stock_record_ids=[stock_record.id],
                partner=partner,
                categories=coupon_categories,
                note=unicode("Autogenerated as part of seat product creation process."),
                course_key=course_id
            )

        return seat

    def create_or_update_coupon_product(self, title, price, stock_record_ids, partner, categories, note,
                                        course_key=None):
        """
        Creates new course coupon products, alternatively updates existing coupon products.
