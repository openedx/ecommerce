"""
Management command to populate initial history for course.
"""
import logging
import time
...
class Command(BaseCommand):
    """
    Populate initial history for 'course'.
    Example usage:
    $ ./manage.py lms generate_initial_history_course --batchsize 1000 --sleep_between 1 --settings=devstack
    """
    ...
    def handle(self, *args, **options):
        ...
        with connection.cursor() as cursor:
            query = u"""
                SELECT
                    t.id
                FROM courses_course t
                LEFT JOIN courses_historicalcourse ht
                    ON t.id = ht.id
                WHERE ht.id IS NULL
                """
