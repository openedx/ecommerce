...
while read line; do
    ...
    docker exec -t edx.devstack.ecommerce bash -c "source /edx/app/ecommerce/ecommerce_env && cd /edx/app/ecommerce/ecommerce/ && python-modernize -w $directories"
    ...
    docker exec -t edx.devstack.ecommerce bash -c "source /edx/app/ecommerce/ecommerce_env && cd /edx/app/ecommerce/ecommerce/ && isort -rc $directories"

    # commit the changes from running isort
    git add $directories || { printf "\n\nERROR: Could not 'git add' directory $directories\n\n"; exit 1; }
    git commit -m "$ticket_number - run python-modernize" || { printf "\n\nERROR: Could not commit files to $branch_name\n\n"; exit 1; }
    ...
    hub pull-request -m "$ticket_number - Run python-modernize on $directories" || { printf "\n\nERROR: Did not successfully create PR for this conversion\n\n"; }
