...
class TestSeatPayment(object):
    ...
    def add_item_to_basket(self, selenium, sku):
        """Adds specific product into basket"""
        ...
        WebDriverWait(selenium, 20).until(
            EC.presence_of_element_located((By.CSS_SELECTOR, ".basket-client-side"))
        )

    def refund_orders_for_course_run(self, course_run_id):
        """Refunds orders against course run"""
        api = EcommerceApi()
    ...
    def get_verified_seat(self, course_run):
        """Returns the Verified Seat for course run"""
        verified_seat = None
